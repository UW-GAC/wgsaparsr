context("test_.parse_min_columns() - unit tests")

test_that(".parse_min_columns() returns expected tibble when parsing snv", {
  ...
  result <- .parse_min_columns(example,
...
test_that(".parse_min_columns() returns expected tibble when parsing indel", {
  example <- dplyr::tibble(
