context("test_.parse_pairs_min() - unit tests")

test_that(".parse_pairs_min() gives error if pair_columns is not a list", {
  ...
  expect_error(.parse_pairs_min(example, "not a list"),
               "pair_columns must be a list")
})

test_that(".parse_pairs_min() gives error when parsing a tripple", {
  example <- dplyr::tibble(
    Eigen_PC_raw = c(".{1}", "-0.164398825586883{1}",
                     ".{5}-0.00506108216253831{4}0.0200792355286427{1}",
                     ".{5}0.636779480039485{1}0.651616016738356{2}", ".{5}",
...
test_that(".parse_pairs_min() returns expected tibble when parsing a single", {
  example <- dplyr::tibble(
    Eigen_PC_raw = c(".{1}", "-0.164398825586883{1}",
                     ".{5}-0.00506108216253831{4}0.0200792355286427{1}",
                     ".{5}0.636779480039485{1}0.651616016738356{2}", ".{5}",
...
test_that(".parse_pairs_min() returns expected tibble when parsing a pair", {
  ...
  target <- dplyr::tibble(
    ...
    Polyphen2_HDIV_score =
      c("0.146", "0.999", "0.243", "0.994", "0.931"),
    Polyphen2_HDIV_pred =
      c("B", "D", "B", "D", "P")
  )
  result <- .parse_pairs_min(example,
                             list(c("Polyphen2_HDIV_score",
                                    "Polyphen2_HDIV_pred")))
  expect_identical(result, target)
})

test_that(".parse_pairs_min() returns expected tibble when parsing two pairs", {
  example <- dplyr::tribble(
    ~Polyphen2_HDIV_score, ~Polyphen2_HDIV_pred, ~Polyphen2_HVAR_score,
    ~Polyphen2_HVAR_pred,
    "0.941;0.941;0.146", "P;P;B", "0.688;0.688;0.138", "P;P;B",
  ...
  target <- dplyr::tibble(
    ...
    Polyphen2_HDIV_pred =
      c("B", "D", "B", "D", "P"),
    Polyphen2_HVAR_score =
      c("0.138", "0.989", "0.136", "0.863", "0.699"),
    Polyphen2_HVAR_pred =
      c("B", "D", "B", "P", "P")
  )
  result <- .parse_pairs_min(example,
